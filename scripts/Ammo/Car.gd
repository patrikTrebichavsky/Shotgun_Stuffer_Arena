			...
			for body in $CarExplosionArea.get_overlapping_bodies():
				if !body.name.contains("Wall"):
					...
					if body == targeted_enemy:
						...
						get_tree().call_group("slots","incremement_value",explo_dmg*slot_recharge_cut)
						$CriticalSound.random_pitch_play()
					else:
						body.got_conditioned(explo_dmg, "burning",true)
						get_tree().call_group("slots","incremement_value",explo_dmg*2*slot_recharge_cut)
	...
	if explo_collider.monitoring:
		for body in $CarExplosionArea.get_overlapping_bodies():
			if !body.name.contains("Wall"):
				...
				get_tree().call_group("slots","incremement_value",burn_damage*slot_recharge_cut)
