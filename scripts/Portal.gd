...
@export var previous_portal : Portal
@export var next_portal : Portal
	
var should_be_hidden = false

var connection_order = 1
...
func _process(delta):
	...
	if visible == false && $RayCast2D.is_colliding():
...
func disapear():	
	...
	if previous_portal.should_be_hidden == true && next_portal.should_be_hidden == true:
		previous_portal.neighbour_portal_opened()
		next_portal.neighbour_portal_opened()
		
	elif previous_portal.should_be_hidden == true:
		previous_portal.neighbour_portal_opened()
		
	elif next_portal.should_be_hidden == true:
		next_portal.neighbour_portal_opened()
